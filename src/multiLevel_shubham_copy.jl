...
function shubhamBoolean(update_matrix::Array{Int,2},
    nInit::Int, nIter::Int, nLevels::Union{Int, Vector{Int}, String}, 
    vaibhav::Bool, turnOffNodes::Array{Int,1},
    ...
    if (length(kdNodes) != 0)
        for i in 1:nInit
            stateList[kdNodes, i] .= sVecList[i][1]
        end
    end
    if (length(oeNodes) != 0)
        for i in 1:nInit
            stateList[oeNodes, i] .= sVecList[i][end]
        end
    end
