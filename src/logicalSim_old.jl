...
"""
    update_all_nodes(f, states_regulators) -> Int

Evaluates a single node's Boolean function `f` using the current
states of its regulators.
"""
function update_all_nodes(F, x, I)
    return [update_single_node(F[i], x[I[i]]) for i in 1:length(x)]
end
...
function num_of_steady_states_asynchronous(
    ...
    if EXACT && isempty(left_side_of_truth_table)
        left_side_of_truth_table = [reverse(collect(t)) for t in product((0:1 for _ in 1:N)...)]
    end
...
"""
    num_of_steady_states_synchronous(
    F, I, N;
    nsim = 500,
    EXACT = false,
    left_side_of_truth_table = [],
...
function num_of_steady_states_synchronous(
    F, I, N;
    nsim = 500,
    EXACT = false,
    left_side_of_truth_table = [],
    ...
    @inbounds for iteration in 1:iterations
        ...
        HAS_ATTRACTOR = Ref(false)
        # Run synchronous updates for at most `search_depth` steps
        for jj in 1:search_depth
            FOUND_NEW_STATE = false # tracks whether the update resulted in a new state
            ...
            if !isnothing(index_ss)
                ...
                # Try one synchronous update round in a random order
                
                fxbin = get(dictF, xbin, nothing)
                # If fxbin is nothing, we don't know the update, hence update. 
                # check if xbin is unvarying at i. if not, the state gets updated and we found a new state.
                if fxbin !== nothing
                    if fxbin != xbin
                    ...
                    # Apply node update rule
                    fx = update_all_nodes(F, x, I)
                    if fx == x
                        fxbin = xbin
                    else
                        # if 0 gets updated to 1, add 2^(N-i) else subtract
                        fxbin = xbin + sum((fx - x).*[2^(N - i) for i in 1:N])
                        FOUND_NEW_STATE = true
                    end
                    x = fx
                    # Cache result
